...
This project **must be run using Python 3.12.2**. Using other Python versions (such as Python 3.13 or Python 3.14) is **not supported** and may cause dependency compatibility issues or prevent the pre-trained machine learning model from loading correctly.
...
> **Important:** Running this project with any Python version other than **3.12.2** is not supported.

> **Important:** Before running the application, go through the provided Google Drive link (<https://drive.google.com/drive/folders/1yFUm-YVLCEQ_iUbU3-XK0On5SE01HkxI?usp=drive_link>), download the `best_air_quality_model.pkl` file, and place it in the `Website/models/` folder.